...
const seact9ine = () => {
    ...
    <h3>차액 정산 송/수신 주기</h3>
    <b>매입요청</b> <br>
    : 거래일자(D)기준 D+1일에 전달 / 365일 전송 (15시 이전)<br>
    <br>
    <b>매입결과</b> <br>
    : 매입요청 검증결과파일은 D+1일에 전달 / 365일 전송 (16시 00분경)<br>
    : 카드사 결과파일은 매입요청 W+4일에 전달 / 365일 전송 (09시 00분경)<br>
    <b>하위사업자 정보 등록 요청</b> <br>
    : 365일 (11시 00분경)<br>
    <br>
    <b>하위사업자 정보 등록 결과</b> <br>
    : 365일 (17시 30분경)<br>
    </div>
    <br>`
}
...
const ksnet = () => {
    ...
    <br>
    <h3>주의사항</h3>
    최종 하위사업자들의 영세/중소 가맹점은 2차 PG사에서 관리해서 보내셔야 합니다.<br>
    매입결과 처리된 건에 대해서만 차액정산 요청이 처리 됩니다.<br>
    차액정산 요청 파일 등록시에 매입처리가 완료된 건이 아닐 경우 처리하지 않습니다. (등록 실패 처리)<br>
    <br>
    일반적인 EDI매입처럼 자동 재처리는 없습니다.<br>
    지정된 시간 이후에 업로드된 파일에 대해서는 다음 영업일에 처리됩니다. <br>
    차액정산 결과 파일 수신일자는 매입결과 수신일 규칙과 동일합니다. (매 영업일 + 공휴일이 아닌 토요일)<br>
    <br>
    <h3>1.2 환급 정산 </h3>
    차액 정산금 지급은 카드사 입금일+1일 입니다. 해당 상점의 정산주기와는 무관합니다.<br>
    </div>
    <br>`
}
