import { Header } from '@/views/headers'
import { getUserLevel } from '@axios'
import corp from '@corp'

export const useRegisterStore = defineStore('mchtRegisterStore', () => {
    const head      = Header('merchandises/bulk-register', '가맹점 대량등록 포멧')
    const levels    = corp.pv_options.auth.levels

    const headers: Record<string, string> = {}
    if (levels.sales5_use && getUserLevel() >= 30) {
        headers['sales5_name'] = levels.sales5_name + ' 상호(X)'
        headers['sales5_fee'] = levels.sales5_name + ' 수수료(X)'
    }
    if (levels.sales4_use && getUserLevel() >= 25) {
        headers['sales4_name'] = levels.sales4_name + ' 상호(X)'
        headers['sales4_fee'] = levels.sales4_name + ' 수수료(X)'
    }
    if (levels.sales3_use && getUserLevel() >= 20) {
        headers['sales3_name'] = levels.sales3_name + ' 상호(X)'
        headers['sales3_fee'] = levels.sales3_name + ' 수수료(X)'
    }
    if (levels.sales2_use && getUserLevel() >= 17) {
        headers['sales2_name'] = levels.sales2_name + ' 상호(X)'
        headers['sales2_fee'] = levels.sales2_name + ' 수수료(X)'
    }
    if (levels.sales1_use && getUserLevel() >= 15) {
        headers['sales1_name'] = levels.sales1_name + ' 상호(X)'
        headers['sales1_fee'] = levels.sales1_name + ' 수수료(X)'
    }
    if (levels.sales0_use && getUserLevel() >= 13) {
        headers['sales0_name'] = levels.sales0_name + ' 상호(X)'
        headers['sales0_fee'] = levels.sales0_name + ' 수수료(X)'
    }
    headers['user_name'] = '가맹점 ID(O)'
    headers['user_pw'] = '가맹점 패스워드(O)'
    headers['trx_fee'] = '가맹점 수수료(X)'
    headers['hold_fee'] = '유보금 수수료(X)'
    headers['mcht_name'] = '상호(O)'
    headers['nick_name'] = '대표자명(O)'
    headers['addr'] = '주소(O)'
    headers['phone_num'] = '휴대폰번호(O)'
    headers['resident_num'] = '주민등록번호(X)'
    headers['business_num'] = '사업자등록번호(X)'
    headers['sector'] = '업종(O)'
    headers['acct_num'] = '계좌번호(O)'
    headers['acct_name'] = '예금주(O)'
    headers['acct_bank_name'] = '입금은행명(O)'
    headers['tax_category_type'] = '사업자 유형(O)'
    headers['custom_id'] = '커스텀 필터(X)'

    if(corp.pv_options.paid.use_collect_withdraw) 
        headers['use_collect_withdraw'] = '모아서 출금 사용여부(X)'
    if(corp.pv_options.paid.use_regular_card) 
        headers['use_regular_card'] = '단골고객 사용여부(X)'
    if(corp.pv_options.paid.use_collect_withdraw)
        headers['collect_withdraw_fee'] = "모아서 출금 수수료(X)"
    if(corp.pv_options.paid.use_withdraw_fee)
        headers['withdraw_fee'] = "출금 수수료(X)"
    if(corp.pv_options.paid.use_pay_verification_mobile)
        headers['use_pay_verification_mobile'] = "결제전 휴대폰 인증(X)"
    if(corp.pv_options.paid.use_multiple_hand_pay)
        headers['use_multiple_hand_pay'] = "다중결제 사용(X)"

    head.sub_headers.value = []
    head.headers.value = head.initHeader(headers, {})
    head.flat_headers.value = head.flatten(head.headers.value)

    const isPrimaryHeader = (key: string) => {
        const keys = ['tax_category_type', 'custom_id']
        return keys.includes(key)
    }

    return {
        head, headers, levels, isPrimaryHeader
    }
})
