...
export const extendRouterValidate = (regex: RegExp, curr_path_name: string) => {
    if (curr_path_name.length) {       
        if (regex.test(curr_path_name)) {
            return true
        }
    }
    return false
...
export const isNavLinkActive = (link: NavLink, router: Router) => {
    ...
    let curr_route_name = router.currentRoute.value.fullPath.replace(/\//g, '-').split('?')[0]
    ...
    const regex_pattern = `^${full_route_name}-(create|edit-[0-9]+|part-[0-9]+|reply-[0-9]+)$`
