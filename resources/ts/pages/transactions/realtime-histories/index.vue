...
const setFianaceVansBalance = async () => {
    ...
    for (let i = 0; i < results.length; i++) {
        if(results[i]['data']['data']['result_cd'] == "0000") {
            finance_vans[i].balance = results[i]['data']['data']['WDRW_CAN_AMT']
        } 
        else {
            finance_vans[i].balance = 0
            const message = finance_vans[i].nick_name+'의 잔고를 불러오는 도중 에러가 발생하였습니다.<br><br>'+results[i]['data']['data']['result_msg']+'('+results[i]['data']['data']['result_cd']+')'
