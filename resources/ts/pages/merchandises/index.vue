...
<template>
    <div>
        <BaseIndexView placeholder="가맹점 상호 검색" :metas="metas" :add="user_info.level >= 35" add_name="가맹점" :is_range_date="true">
            <template #filter>
                    ...
                    <template v-for="(_header, _key, _index) in head.headers" :key="_index">
                        <template v-if="head.getDepth(_header, 0) != 1">
                            <td v-for="(__header, __key, __index) in _header" :key="__index"
                                v-show="!(__header?.hidden as boolean)" class='list-square'>
                                <span>
                                    {{ item[_key][__key] }}
                                </span>
                            </td>
