...
const batchRemove = async() => {
    if (await alert.value.show(`정말 ${selected.value.length}개의 이력을 삭제하시겠습니까?`)) {
        const r = await request({ url: `/api/v1/manager/salesforces/fee-change-histories/batch-remove`, method: 'delete', data: {
...
<template>
    <BaseIndexView placeholder="영업라인 상호 검색" :metas="[]" :add="false" add_name="가맹점" :date_filter_type="DateFilters.DATE_RANGE">
                    ...
                    <template v-else>
                        <td v-show="_header.visible" class='list-square'>
                            ...
                            <span v-else-if="_key == 'remove'">
                                <VBtn size="small" type="button" color="error" @click="remove('/salesforces/fee-change-histories', item, false)">
